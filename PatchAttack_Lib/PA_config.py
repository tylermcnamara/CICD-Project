...
def configure_PA(t_name, t_labels, 
    ...
    # TPA attack dirs
    TPA_attack_dirs = []
    # Get a relative path from t_name to the texture_template_dir (cross-platform)
    rel_path = os.path.relpath(PA_cfg.texture_template_dirs[0], start=PA_cfg.t_name)

    for agent_index in range(PA_cfg.n_agents):
        attack_dir = os.path.join(
            'tgt' if PA_cfg.target else 'ntgt',
