...
class MPA_agent(robot):
    ...
    def create_mask(points, distributed_area=False, distributed_mask=False, H=224, W=224):
        ...
        for o in range(num_occlu):
            p_combo.append(
                torch.index_select(points, dim=1, index=torch.LongTensor([o * p_l + 0, o * p_l + 2]).cuda(torch_cuda))
                .sort(dim=1, descending=False)[0])
            p_combo.append(
                torch.index_select(points, dim=1, index=torch.LongTensor([o * p_l + 1, o * p_l + 3]).cuda(torch_cuda))
                .sort(dim=1, descending=False)[0])
                ...
                if H > W:
                    ...
                    for item in range(bs):  # iterate over each sample in the batch
                        for o in range(num_occlu):  # iterate over each occlusion region
                            ...
                            mask[item, o][
                            p_combo[item][o * 4 + 0] * self.shrink:p_combo[item][o * 4 + 1] * self.shrink + 1,
